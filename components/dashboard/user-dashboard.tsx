...
export async function UserDashboard({ userId }: UserDashboardProps) {
  ...
  const statusColors: Record<
    string,
    { bg: string; text: string; dot: string; light: string }
  > = {
    active: {
      bg: "bg-green-50",
      text: "text-green-700",
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        ...
        <p className="text-base text-slate-500 leading-relaxed">
          Welcome back. Monitor your devices and maintenance status in real-time
        </p>
      </div>
            ...
            >
              ...
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  ...
                  <div
                    className={`p-3 rounded-xl bg-gradient-to-br ${stat.color} bg-opacity-10 group-hover:bg-opacity-20 transition-all duration-300`}
                  >
                    <Icon
                      className={`h-6 w-6 text-transparent bg-clip-text bg-gradient-to-br ${stat.color}`}
                    />
                  </div>
                </div>
              </CardHeader>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription className="text-slate-600 text-sm">
                  Your registered scanners and printers (
                  {myDevices?.length || 0} total)
                </CardDescription>
              </div>
            ...
            {myDevices && myDevices.length > 0 ? (
                    ...
                    >
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-4 flex-1">
                          <div
                            className={`p-3 ${colors.light} rounded-lg border border-slate-200 shadow-sm group-hover:shadow-md transition-all duration-200`}
                          >
                        ...
                        <div className="flex items-center gap-3">
                          <div className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-full border border-slate-200/50">
                            ...
                            ></div>
                            <span
                              className={`text-xs font-semibold ${colors.text} capitalize`}
                            >
                              {device.status}
                            </span>
                          </div>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-slate-500 mb-5">
                  Start by adding your first device to get started
                </p>
                <Link href="/dashboard/my-devices">
                  <Button
                    variant="outline"
        ...
        {/* <Card className="border border-slate-200/50 bg-white shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden lg:col-span-1">
          <div className="border-b border-slate-200/50 bg-gradient-to-r from-slate-50/80 to-green-50/50 px-6 py-5 backdrop-blur-sm">
            ...
            <CardDescription className="text-slate-600 text-sm">
              AI-powered predictions and health
            </CardDescription>
          </div>
          <CardContent className="pt-6">
              ...
              <div className="space-y-3">
                <div className="px-3 py-2 bg-blue-50 border border-blue-100 rounded-lg">
                  <p className="text-xs font-medium text-blue-600">
                    ⚠️ {myPredictions.length} potential issue(s) detected
                  </p>
                </div>
                {myPredictions.slice(0, 3).map((prediction: any) => {
                  ...
                  return (
                    <div
                      key={prediction.id}
                      className={`p-3 rounded-lg border ${config.border} ${config.bg} hover:shadow-sm transition-all group cursor-pointer`}
                    >
                      <div className="flex items-start justify-between gap-2 mb-1.5">
                        ...
                        <Badge
                          className={`${config.badge} border-0 whitespace-nowrap text-xs`}
                        >
                          {riskScore}%
                        </Badge>
                      </div>
                      ...
                      <div className="mt-2 w-full h-1 bg-slate-200 rounded-full overflow-hidden">
                        <div
                          className={`h-full bg-gradient-to-r ${config.dot === "bg-red-500" ? "from-red-500" : config.dot === "bg-amber-500" ? "from-amber-500" : "from-yellow-500"} to-orange-500 transition-all`}
