...
import { Crown, Database, Cpu, ClipboardList, Sparkles, FileSpreadsheet } from 'lucide-react'
...
export async function PremiumUserDashboard({ userId }: PremiumUserDashboardProps) {
  ...
  const completedCount = (datasets || []).filter((d: any) => d.status === 'completed').length

  const stats = [
    { label: 'My Datasets', value: datasetCount || 0, icon: Database, color: 'from-amber-500 to-orange-500' },
    { label: 'Completed Analyses', value: completedCount, icon: Sparkles, color: 'from-purple-500 to-fuchsia-500' },
    { label: 'My Devices', value: deviceCount || 0, icon: Cpu, color: 'from-blue-600 to-blue-500' },
    { label: 'Pending Requests', value: pendingRequests || 0, icon: ClipboardList, color: 'from-emerald-500 to-teal-500' },
  ]
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 dark:text-slate-400">
            Upload your own datasets and let our admins run the predictive model on your data.
          </p>
        </div>
        ...
        <CardContent>
          {datasets && datasets.length > 0 ? (
              ...
              {datasets.map((d: any) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <Link href={`/dashboard/my-datasets/${d.id}`}>
            ...
            <div className="text-center py-10 text-slate-500">
              ...
              <p className="text-sm">Upload your first dataset to get predictions.</p>
            </div>
          )}
        </CardContent>
      </Card>
